...
SVMmodel_rbf = fitcsvm(X_train_original_tuning, y_train_original_tuning, 'KernelFunction', 'rbf',...
...
best_boxconstraint = SVMmodel_rbf.HyperparameterOptimizationResults.XAtMinObjective.BoxConstraint;
best_kernelscale = SVMmodel_rbf.HyperparameterOptimizationResults.XAtMinObjective.KernelScale;
...
% best_boxconstraint = 250.8, best_kernelscale = 166.34 (24000 training)

best_SVM_model = fitcsvm(X_train_original, y_train_original, 'KernelFunction', 'rbf',...
...
confusion_matrix.Title = 'Confusion matrix - rbf kernel - original test data - original training';
confusion_matrix.FontName = 'Cambria';
f1_score = f1_score(y_label, y_predicted);
cohens_kappa = cohens_kappa(y_label, y_predicted);

%% f1_score = 0, cohens kappa = 0, accuracy = 77.8%, training time = 196.5)
% overfits, always predicts class 0 (no default)
% tuning 5000
% training 24000

fprintf('The f1 score of the best rbf model is: %f\n ', f1_score)
fprintf('The cohens kappa of the best rbf model is: %f\n ', cohens_kappa)
fprintf('The accuracy of the best rbf model is: %f\n ', test_accuracy)